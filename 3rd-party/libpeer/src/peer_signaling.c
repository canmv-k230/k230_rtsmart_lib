...
typedef struct PeerSignaling {
  ...
  peer_signaling_custom_rpc_cb custom_rpc_cb;
  void* custom_rpc_userdata;
...
static void peer_signaling_on_pub_event(const char* msg, size_t size) {
  ...
  do {
    ...
    if (strcmp(item->valuestring, RPC_METHOD_OFFER) == 0) {
      ...
      if (g_ps.custom_rpc_cb) {
        int handled = g_ps.custom_rpc_cb(item->valuestring, cJSON_GetObjectItem(req, "params"),
                                         id, &result, &error, g_ps.custom_rpc_userdata);
        if (handled != 0) {
...
void peer_signaling_set_custom_rpc_handler(peer_signaling_custom_rpc_cb cb, void* userdata) {
  g_ps.custom_rpc_cb = cb;
  g_ps.custom_rpc_userdata = userdata;
}
...
int peer_signaling_publish(const char* topic, const char* message) {
  ...
  if (g_ps.proto != 0) {
    LOGW("peer_signaling_publish: not supported in HTTP mode");
    return -1;
  }

  if (g_ps.mqtt_fixed_buf.pBuffer == NULL) {
    LOGW("peer_signaling_publish: MQTT not connected");
    return -1;
  }
  ...
  if (status != MQTTSuccess) {
    LOGE("peer_signaling_publish failed: topic=%s status=%s",
         topic, MQTT_Status_strerror(status));
    return -1;
  }

  LOGD("peer_signaling_publish: topic=%s len=%zu", topic, strlen(message));
  return 0;
}
