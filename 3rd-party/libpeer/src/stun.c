...
void stun_parse_msg_buf(StunMessage* msg) {
  ...
  msg->stunmethod = ntohs(header->type);
  // Extract method bits: M0-M3 at bits 0-3, M4-M6 at bits 5-7, M7-M11 at bits 9-13
  // Mask out class bits (C0 at bit 4, C1 at bit 8) and rearrange method bits
  msg->stunmethod = ((msg->stunmethod & 0x3E00) >> 2) |
  ...
  while (pos + sizeof(StunAttribute) <= length) {
    ...
    switch (attr_type) {
      ...
      case STUN_ATTR_TYPE_ERROR_CODE:
        {
          ...
          LOGE("STUN Error Code: %d", msg->error_code);
        }
        break;
      ...
      case STUN_ATTR_TYPE_DATA:
        msg->data_offset = pos + sizeof(StunAttribute);
      ...
      case STUN_ATTR_TYPE_SOFTWARE:
        break;
      default:
        LOGD("Unknown Attribute Type: 0x%04x", attr_type);
        break;
    }
...
int stun_msg_finish(StunMessage* msg, StunCredential credential, const char* password, size_t password_len) {
  ...
  header->length = htons(header_length + 24);
  ...
  stun_attr = (StunAttribute*)(msg->buf + msg->size);
  header->length = htons(header_length + 24 + 8);
